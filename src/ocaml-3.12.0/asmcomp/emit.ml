...
let emit_instr i =
    ...
    | Lop(Ialloc n) ->
        if !fastcode_flag then begin
          ...
          (emit_string "	sub	alloc_ptr, alloc_ptr, r12\n");
          (emit_string "	cmp	alloc_ptr, alloc_limit\n");
          (record_frame i.live; emit_string "	blcc    "; emit_symbol "caml_call_gc"; emit_char '\n');
          (emit_string "	add	"; emit_reg i.res.(0); emit_string ", alloc_ptr, #4\n");
          4 + ni
  ...
  | Lswitch jumptbl ->
    ...
    | Lpushtrap ->
        ...
        (emit_string "	stmfd	sp!, {trap_ptr, lr}\n");
        (emit_string "	mov	trap_ptr, sp\n"); 2
    | Lpoptrap ->
        (emit_string "	ldmfd	sp!, {trap_ptr, lr}\n");
        stack_offset := !stack_offset - 8; 1
    | Lraise ->
        (emit_string "	mov	sp, trap_ptr\n");
        (emit_string "	ldmfd	sp!, {trap_ptr, pc}\n"); 2
...
let begin_assembly() =
  (emit_string "trap_ptr .req r11\n");
  (emit_string "alloc_ptr .req r8\n");
  (emit_string "alloc_limit .req r10\n");
